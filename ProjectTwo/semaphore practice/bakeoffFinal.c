...
// Function to get a map thread to a color based on its ID
const char* getThreadColor(int id) {
    // six possible colors for now can expand
    switch (id % 6) {
        case 0: return RED;
        case 1: return GREEN;
...
void* bakersBake(void* arg) {
    // Allows use to gain access to the info about the threads and the shared data
    ThreadArgs* args = (ThreadArgs*)arg; 
    // Assign pantry and refrigerator numbers to the current thread. 
    ...
    const char* color = getThreadColor(args->id);
    printf("%sBaker %d: HAS FINISHED!!!.\n%s", color, args->id, RESET);
    return NULL;
}
...
int main() {
    ...
    int numBakers;
    printf("How many bakers will compete? ");
    scanf("%d", &numBakers);

    //Allocate memory for threads and arguments
    pthread_t* baker = malloc(numBakers* sizeof(pthread_t));
    // Allows us to pass info about our current thread and data when passing function to pthread create
    ThreadArgs* args = malloc(numBakers * sizeof(ThreadArgs));
    Kitchen kitchen; 
    initKitchen(&kitchen);
